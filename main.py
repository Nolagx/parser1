...
def run_passes(tree, passes):
    for cur_pass in passes:
        if isinstance(cur_pass, Visitor) or isinstance(cur_pass, Visitor_Recursive) or \
                isinstance(cur_pass, Interpreter):
            cur_pass.visit(tree)
        elif isinstance(cur_pass, Transformer):
            tree = cur_pass.transform(tree)
        elif isinstance(cur_pass, Converter):
